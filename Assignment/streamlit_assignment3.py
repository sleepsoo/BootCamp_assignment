...
df = pd.read_csv(
    '/Users/pro/Python/MetaCode/Assignment/data/sunspots_for_prophet.csv')
...
ax.plot(forecast["ds"], forecast["yhat"], color='red',
...
merged = pd.merge(df[['ds', 'y']], forecast[['ds', 'yhat']],
                  on='ds', how='inner')
merged['residual'] = merged['y'] - merged['yhat']

# TODO: residual 시계열을 시각화하세요.
fig4, ax2 = plt.subplots(figsize=(14, 4))
plt.plot(merged['ds'], merged['residual'], color='purple',
         marker='o', linestyle='-', label='Residual')
...
# streamlit run streamlit_assignment3.py
