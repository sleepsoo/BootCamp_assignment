...
def plot_advanced_sunspot_visualizations(df, sunactivity_col='SUNACTIVITY',
    ...
    if len(data) > 0:  # 데이터가 있는지 확인
        ...
        axs[0, 1].hist(data, bins=30, density=True, alpha=0.6,
                       color='gray', label='Histogram')
        axs[0, 1].plot(xs, density(xs), color='red',
    ...
    if len(years_clean) > 1:  # 회귀선을 그리기 위해 최소 2개 이상의 데이터 필요
        axs[1, 1].scatter(years_clean, sun_activity_clean,
                          s=point_size, alpha=point_alpha, label='Data Points')
        coef = np.polyfit(years_clean, sun_activity_clean, trend_degree)
        trend = np.poly1d(coef)

        # 추세선을 그리기 위한 x 값 생성
        x_trend = np.linspace(years_clean.min(), years_clean.max(), 100)
        axs[1, 1].plot(x_trend, trend(x_trend), color='red',
...
try:
    # 데이터 로드
    df = load_data('Assignment/data/sunspots.csv')
...
# streamlit run streamlit_assignment2.py
